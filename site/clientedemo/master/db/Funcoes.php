...
$dadpro 			= $connect->query("SELECT * FROM produtos, setores WHERE idu =  '$cod_id' and setores.setor_id = produtos.setor_id ORDER BY nome ASC");

$func               = $connect->query("SELECT * FROM funcionarios, setores WHERE idu='$cod_id' and setores.setor_id = funcionarios.setor_id ORDER BY nome ASC");

$dadSet             = $connect->query("SELECT * FROM setores WHERE idu_setor =  '$cod_id' ORDER BY setor_id ASC");
...
if(isset($_POST["cadWallet"]))  {
	...
	$cadfunc = $connect->query("INSERT INTO wallet (idu, numero) VALUES ('$cod_id','$numero')");
	
	if ( $cadfunc ) { header("location: wallet.php?ok=ok"); exit; } else { header("location: wallet.php?erro=erro"); exit; }
...
if(isset($_POST["delWallet"]))  {
	...
	$delb = $connect->query("DELETE FROM wallet WHERE id='".$_POST['delWallet']."'");
	
	if ( $delb ) { header("location: wallet.php?ok=ok"); exit; } else { header("location: wallet.php?erro=erro"); exit; }
...
$cad_setor 	= $_POST['cad_setor'];

$cadfunc = $connect->query("INSERT INTO funcionarios (idu, nome, login, senha, acesso, setor_id) VALUES ('$cod_id','$cad_nome','$cad_login','$cad_senha','$cad_acesso','$cad_setor')");
...
if(isset($_POST["cadSubcate"]))  {

	$cad_nome 		= $_POST['Subcad_nome'];
	
	$cad_pos 		= $_POST['Subcad_posicao'];

	$subcat_pai = $_POST['cad_cat'];
	
	if ($_FILES['pic']['size'] >= 1) {
	
	$imagem 		= $_FILES['pic']['name'];
	
	$dir 			= "../img/categoria/Sub/";
	...
	$cadSubcat 		= $connect->query("INSERT INTO sucategoria(Descricao, url, idu,categoria_pai) VALUES ('$cad_nome','$nova_imagem','$cod_id','$subcat_pai')");
	
		if ( $cadSubcat ) { header("location: categorias.php?ok=ok"); exit; } else { header("location: categorias.php?erro=erro"); exit; }
...
if(isset($_POST["deletarSubcat"]))  {

	$delb 			= $connect->query("DELETE FROM sucategoria WHERE id='".$_POST['deletarcat']."'");
...
if(isset($_POST["editSubcate"]))  {
	...
	$editarcad = $connect->query("UPDATE sucategoria SET descricao='$cad_nome', url='$nova_imagem' WHERE id='$cad_iduc'");
...
$arrayCat = explode('-',$_POST['cad_cat']);
...
$novocad = $connect->query("INSERT INTO produtos(nome, valor, ingredientes, categoria, idu, foto, destaques, setor_id, tempo_padrao, subcategoria) VALUES ('$cad_nome','$cad_val','$cad_ing','$cad_cat','$cod_id','$nova_imagem','$cad_dest','$cad_setor','$cad_tempo','$sub_cat')");
...
if(isset($_POST["cadSetor"]))  {

	$cad_nome 		= $_POST['cad_nome'];
	
	
	
	$editarcad = $connect->query("INSERT INTO setores (descricao, idu_setor) VALUES('$cad_nome',$cod_id);");
	
	if ( $editarcad ) { header("location: setores.php?ok=123"); exit; } else { header("location: setores.php?erro=erro"); exit; }
	...
	if(isset($_POST["deletarSetor"]))  {

		$delb 			= $connect->query("DELETE FROM setores WHERE setor_id='".$_POST['deletarSetor']."'");
		
		
		if ( $delb ) { header("location: setores.php?ok=ok"); exit; } else { header("location: setores.php?erro=erro"); exit; }
		...
		if(isset($_POST["editSetor"]))  {
			...
			$editarcad = $connect->query("UPDATE setores SET descricao='$cad_nome' WHERE setor_id='$cad_iduc'");
